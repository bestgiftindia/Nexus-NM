        ...
        <div class="col-xxl-12">
            <div data-table data-table-rows-per-page="8" class="card">
                <div class="card-header border-light justify-content-between">
                    <div>
                        <h2 class="card-title d-block">Edit Role</h2>
                        <p class="text-muted mb-0">
                            Update the role details and manage its assigned permissions.
                        </p>
                    </div>
                ...
                <form method="post" action="{{ route('account.roles.update', ['role' => $role->id]) }}"
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror " name="name"
                                id="name" value="{{ old('name', $role->name) }}" placeholder="Enter Role Name">
                            @error('name')
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="is_publish" id="is_publish"
                                ...
                                <option value="1" @selected(old('is_publish', $role->is_publish) == 1)>Publish</option>
                                <option value="0" @selected(old('is_publish', $role->is_publish) == 0)>Unpublish</option>
                            </select>
                            @error('is_publish')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="permissions[]" class="select2 form-control select2-multiple" data-toggle="select2"
                                ...
                                @foreach ($permissions as $permission)
                                    <option value="{{ $permission->name }}" @selected(in_array($permission->name, old('permissions', $rolePermissions)))>
