        ...
        <div class="col-xxl-12">
            <div data-table data-table-rows-per-page="8" class="card">
                <div class="card-header border-light justify-content-between">
                    <div>
                        <h2 class="card-title d-block">Edit Permission</h2>
                        <p class="text-muted mb-0">
                            Update the permission details and manage access within the system.
                        </p>
                    </div>
                ...
                <form method="post" action="{{ route('account.permissions.update', ['permission' => $permission->id]) }}"
                    class="card-body border-top">
                    @csrf
                    @method('PATCH')
                    <div class="row g-3">
                        <div class="col-md-10">
                            <label for="lastname" class="form-label">Permission Name <span
                                    class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                                id="name" value="{{ old('name', $permission->name) }}"
                                placeholder="Enter Permission Name">
                            <small class="text-secondary">(NOTE: Please enter the permission name in the format
                                <b>user-lists</b>. If you
                        ...
                        <div class="col-md-2">
                            <label for="lastname" class="form-label">Permission Status <span
                                    class="text-danger">*</span></label>
                            <select name="is_publish" id="is_publish"
                                class="form-select @error('is_publish') is-invalid @enderror">
                                <option value="1" @selected(old('is_publish', $permission->is_publish) == 1)>Publish</option>
                                <option value="0" @selected(old('is_publish', $permission->is_publish) == 0)>Unpublish</option>
                            </select>
                            @error('is_publish')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
